...
static void paddle_move(WINDOW *win,
                        struct paddle_t *paddle,
                        const struct ball_t *ball)
{
  if (paddle->vx == 0 || ((ball->x == paddle->x + paddle->vx ||
...
void bpaddle_move(WINDOW *win,
                  struct paddle_t *bpaddle,
                  const struct ball_t *ball)
{
